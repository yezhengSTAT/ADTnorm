## fdaNorm transformation
require(flowStats)

fdanorm_transform <- function(cell_x_adt = NULL, cell_x_feature = NULL, parameter_list = NULL) {
    ...
    adt_fdanorm <- warpSet(file_fcs, colnames(cell_x_adt), monwrd = TRUE) %>% as.flowSet() 

    out <- c()
    for (i in 1:length(adt_fdanorm)) {
        out <- adt_fdanorm[[i]] %>%
            exprs() %>%
            rbind(out, .)
    }
