...
#' @examples
#' \dontrun{
#' get_peak_midpoint(cell_x_adt, cell_x_feature, adt_marker_select = "CD3", neg_candidate_thres = asinh(6/5+1))
#' }
# require(flowStats)
# require(dplyr)
                    ...
                    if (adt_marker_index %in% bimodal_marker_index) {
                        if (length(peak_info$midpoint) == 2) {
                            ...
                            if (length(peak_info$midpoint) == 2) {
                                ...
                                if(is.na(peak_info$midpoint) || (length(peak_info$midpoint) >= 2 && peak_info$midpoint[2] - peak_info$midpoint[1] < 0.5)){
                            ...
                            peak_info$midpoint = peak_info$midpoint
                            if(is.na(peak_info$midpoint) || (length(peak_info$midpoint) >= 2 && peak_info$midpoint[2] - peak_info$midpoint[1] < 0.5)){
                                    peak_info = flowStats:::curvPeaks(
                                        x = fres1,
                                        dat =  adt_expression,
