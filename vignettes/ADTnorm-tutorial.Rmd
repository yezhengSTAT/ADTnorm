...
For a quick start of using ADTnorm, we will treate all the cells from one study as one sample and normalize across studies.
...
```valley_density_adjust```: Parameter for `density` function: bandwidth used is adjust*bw. This makes it easy to specify values like half the default bandwidth. Lower value such as 0.5 or 1 can lead to more accurate valley location but more prone to local minimal variations. Larger value such as 3 or 5 can make the valley more consistent across samples.
